...
import { Phone, Mail, MapPin, Target, Zap, Leaf, Shield, Heart, Menu, X, Users } from 'lucide-react'; // 🚨 CORRECTION: Ajout de Users ici
        ...
        <header className="fixed top-0 left-0 right-0 z-50 shadow-lg" style={{ backgroundColor: BG_DARK }}>
            <div className="max-w-7xl mx-auto px-4 sm:px-8 py-3 flex justify-between items-center text-sm">
                ...
                <nav className="hidden md:flex space-x-6 text-white text-base font-medium">
                    ...
                    <Link href="/solutions" className="hover:text-gray-400 transition-colors">Nos solutions</Link>
            ...
            {isOpen && (
                <div className="md:hidden absolute w-full bg-dark/95 backdrop-blur-sm shadow-xl pb-4">
                    <nav className="flex flex-col space-y-3 px-8 pt-2 pb-4 text-lg font-medium border-t border-gray-700">
                        <Link href="/" className="text-white hover:text-gray-400 transition-colors" onClick={toggleMenu}>Accueil</Link>
                        <Link href="/solutions" className="text-white hover:text-gray-400 transition-colors" onClick={toggleMenu}>Nos solutions</Link>
                        <Link href="/about" className="text-white hover:text-gray-400 transition-colors" onClick={toggleMenu}>À propos</Link>
                        <Link href="/blog" className="text-white hover:text-gray-400 transition-colors" onClick={toggleMenu}>Blog</Link>
...
const Footer = () => {
    return (
        <footer className="bg-dark text-white py-12 px-8 border-t border-gray-700">
...
export default function Home() {
    ...
    const [numbersRef, isNumbersVisible] = useAnimateOnScroll();
    const [aboutRef, isAboutVisible] = useAnimateOnScroll();
    const [solutionsRef, isSolutionsVisible] = useAnimateOnScroll();
    const [impactRef, isImpactVisible] = useAnimateOnScroll();
    const [missionRef, isMissionVisible] = useAnimateOnScroll();
    const [testimonialsRef, isTestimonialsVisible] = useAnimateOnScroll();
    const [contactRef, isContactVisible] = useAnimateOnScroll();
  ...
  return (
    <div style={{ backgroundColor: BG_DARK }} className="flex flex-col min-h-screen text-white font-sans overflow-x-hidden pt-[68px]"> 
      ...
      >
          ...
          <div className="relative max-w-4xl mx-auto px-8 space-y-6 z-20">
      ...
      <section ref={aboutRef} style={{ backgroundColor: BG_DARK }} className={`py-20 px-8 ${isAboutVisible ? 'animate-fade-in' : 'opacity-0'}`}>
      ...
      <section ref={solutionsRef} style={{ backgroundColor: BG_GREEN_LIGHT, color: TEXT_DARK }} className={`py-20 px-8 ${isSolutionsVisible ? 'animate-fade-in' : 'opacity-0'}`}>
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="bg-white p-6 shadow-lg flex space-x-6 transform hover:-translate-y-2 transition-transform duration-300 animate-slide-up-delay-1">
                  <div className="relative w-16 h-16 flex-shrink-0">
                      <Image 
                          src="/assets/solution-1-icon.jpg" 
                          alt="Icône Robotique" 
                          fill 
                          sizes="64px"
                      />
                  ...
                  <div>
                      ...
                      <Link href="/analyse-sols" style={{ color: GREEN_PRIMARY }} className="font-semibold flex items-center group">
              ...
              <div className="bg-white p-6 shadow-lg flex space-x-6 transform hover:-translate-y-2 transition-transform duration-300 animate-slide-up-delay-2">
                  <div className="relative w-16 h-16 flex-shrink-0">
                      <Image 
                          src="/assets/solution-2-icon.jpg" 
                          alt="Icône Intelligence IA" 
                          fill
                          sizes="64px"
                      />
                  </div>
